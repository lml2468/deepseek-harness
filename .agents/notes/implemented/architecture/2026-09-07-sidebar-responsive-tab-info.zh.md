...
框架先缩小右列，再关闭右列，最后才缩小会话区，以保护会话区的 400px 宽度。右列首次打开偏好为视口的 35%，此后按像素保留，下限为 300px，上限为视口的 70%。在宽度至少为 1024px 时，左列保持自身偏好。关闭是被记录的状态：变宽不会打开右栏，用户动作或显式 Session API 可以打开。会话作用域的 Sidebar store 在本地持久化布局，因此刷新后会恢复已记录的展示状态。默认宽度与共享 header 几何由[右侧 Sidebar chrome 几何](../bug-fix/2026-09-09-right-sidebar-chrome-geometry.zh.md)进一步明确。
...
标签扩展使用框架注入的读取接口，自身 store actions 与 `tab.actions` 保持分离。布局、seat 与停靠测试覆盖列宽让步、显式重开、正文与标题可见性、标签生命周期、水平放置区与分割比例；浏览器测试覆盖组装后的应用。紧凑移动端控件不属于本决策。
