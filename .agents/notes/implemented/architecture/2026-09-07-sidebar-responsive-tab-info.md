...
The frame protects 400px for the conversation by shrinking the right column, then closing it before shrinking the conversation. Its first-open preference is 35% of the viewport, retained thereafter in pixels, with a 300px floor and 70% viewport ceiling. The left column keeps its preference at widths of at least 1024px. Closing is recorded state: widening never opens it, while a user action or explicit Session API may. The session-scoped Sidebar store persists layout locally, so refresh restores the recorded presentation. The default width and shared header geometry are refined by [right Sidebar chrome geometry](../bug-fix/2026-09-09-right-sidebar-chrome-geometry.md).
