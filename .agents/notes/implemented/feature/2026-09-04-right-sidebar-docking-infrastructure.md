...
**The expand button as a `conversation.session.header.utilities` entry.** Tried after the rail. Rejected on review: as a list entry it sat inside the utilities row, so it was not at the header's true corner. A dedicated corner seat with a persistent fixed-size toggle keeps the Session log control stable and leaves the collapse action available; [right Sidebar chrome geometry](../bug-fix/2026-09-09-right-sidebar-chrome-geometry.md) owns this refinement.
...
- The Sidebar has one persistent header-corner toggle: the conversation keeps its full width and its scrollbar at its edge while the panel is collapsed, and the same fixed-size control remains available to collapse the expanded panel without moving other header content.
