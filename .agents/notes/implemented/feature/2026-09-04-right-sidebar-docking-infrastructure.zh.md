...
**展开按钮作为 `conversation.session.header.utilities` 的一个 list 条目。** rail 之后的一试。评审否决：作为 list 条目它坐在工具区行内，不在头部真正的角落。专用角落坑位中的持久固定尺寸开关让 Session log 控件保持稳定，并始终提供收起操作；[右侧 Sidebar chrome 几何](../bug-fix/2026-09-09-right-sidebar-chrome-geometry.zh.md)负责这项修正。
...
- 布局可撤销、在本地持久化并按会话隔离；刷新会恢复 tab 与展示状态。undo 只能经 `@internal` 服务方法触达；产品不显示历史控件。
...
- Sidebar 始终在头部角落保留一个开关：面板收起时会话区保持全宽、滚动条停在自己的边缘，面板展开时同一个固定尺寸控件仍可用于收起，且头部其余内容不动。
