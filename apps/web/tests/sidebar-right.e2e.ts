...
/** Clear persisted presentation state before an independent gesture case. */
async function resetSidebar(page: Page): Promise<Locator> {
  await page.evaluate(() => {
    for (let index = localStorage.length - 1; index >= 0; index -= 1) {
      const key = localStorage.key(index)
      if (key?.startsWith('dsh.conversation.workbench.v1.') === true) localStorage.removeItem(key)
    }
  })
  await page.reload({ waitUntil: 'load' })
  const settled = page.getByRole('treeitem', { name: /Show the right sidebar\./u }).first()
  await settled.click()
...
describe('web e2e: shipped right Sidebar', () => {
  ...
  describe('with a settled session', () => {
    ...
    it('§9.7 restores the current Session surface after a reload', async () => {
      ...
      const column = page.locator('[data-rightbar-col]')
      await column.waitFor({ state: 'attached', timeout: 15_000 })
      // Workbench state is persisted presentation data. A reload restores the
      // current Session's open surface and tabs without copying file content.
      await expect.poll(async () => await frame.getAttribute('data-rightbar-collapsed')).toBe(null)
      await expect.poll(async () => await column.locator('[data-sidebar-right-open]').count()).toBe(1)
      await expect.poll(async () => await tabTitles(column)).toEqual(['Files'])
      expect(await expandOf(page).count()).toBe(0)
