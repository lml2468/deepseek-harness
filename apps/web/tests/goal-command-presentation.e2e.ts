...
describe('web e2e: /goal human transcript presentation', () => {
  ...
  it('completes with Tab and shows the bare input and result without a model turn', async () => {
    ...
    await expect.poll(() => page.getByText('What can I help you with?', { exact: false }).count(), {
      timeout: 15_000,
    }).toBe(1)
    ...
    expect(await page.getByText('What can I help you with?', { exact: false }).count()).toBe(0)
