...
`ctx.composerMenuActions` registers ordered entries in the resident `+` menu. Each entry computes visibility and its disabled reason from the current composer context and invokes its owning capability; the registry does not store capability state. File attachment, reference, and command packages use this route. Persistent selections continue to use `conversation.input.left` or `conversation.input.right`.
