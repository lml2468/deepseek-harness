...
面板没有标题行。它的两个控件——形态切换与折叠按钮——搭在套件 chrome 席位上，位于右上格 tab 条的最末端，因此 tab 条就是面板的整条上边，并与会话标题栏共用 56px 基线。每条 tab 条从左到右读作：作为胶囊、各带关闭按钮的 tab，添加控件（只在该格没有引导 tab 时绘制；它通过 `ctx.sidebarRight.openTab` 在该格打开引导页），该格的分栏控件，以及右上格里的两个面板控件。窄格里只有 chip 让位；其后的控件从不收缩或被裁切。

<a id="the-expand-button"></a>
## 展开按钮

已连接 Session 的会话 header 角落席位（`conversation.session.header.corner`，在工具组右缘之外，与 Session 日志控件齐平）始终渲染面板开关。它的图形是左侧 sidebar 折叠图标的镜像，无障碍名称根据当前状态表达展开或收起操作。它与面板共用一个存储，因为 slot 运行时允许两个同作用域席位共用一个 handle。固定尺寸的控件让 header 行在两种状态下保持稳定；折叠的 Sidebar 不占用轨条或列宽，转录的滚动条留在列的边缘。没有 Session 就没有按钮也没有面板。
